...
# gen_data <- function(N, vars=c('y', 'x'), coef=0, mean=0, sd=1) {
#     obs <- rep(1:N)
#
#     if (typeof(coef) == 'list') {
#         if ('b0' %in% names(coef)) {
...
# }
#
# power_lm1 <- function(N, vars=c('y', 'x'), form='y ~ x', coef=0, mean=0, sd=1,
#     n.sims=5000, seed=runif(1)*1000) {
#
...
#         allTerms <- 'intercept'
#     }
#     allTerms <- c(allTerms, attr(terms(formula), 'term.labels'))
#
#     dimnames <- 'sim'
#     for (v in 1:length(vars)) {
#         dimnames <- c(dimnames, paste0(vars[v], '.mean'), paste0(vars[v], '.sd'))
#     }
#
#     for (t in 1:length(allTerms)) {
#         dimnames <- c(dimnames, paste0(allTerms[t], '.est'),
#             paste0(allTerms[t], '.se'), paste0(allTerms[t], '.p'),
#             paste0(allTerms[t], '.sig'))
#     }
#
#     output <- matrix(nrow=n.sims, ncol=length(dimnames), dimnames=list(NULL,
#         dimnames))
#     set.seed(seed)
#     for (s in 1:n.sims) {
#         sim <- gen_data(N, vars, coef, mean, sd)
