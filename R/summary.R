...
#'
#' \code{summary.pwr_sim} calculates a summary of a given variable across all,
#' or a portion of, the simulations.
#'
#' @param sim A list of class 'pwr_sim' generated by the function \code{pwr_sim}.
...
#' @seealso \code{\link{pwr_sim}}
#' @examples
#' # will calculate the mean for all 'sig' values in all simulations where N=200,
#' # and all simulations where N=300
#' summary(power, 'sig', params=list(N=c(200, 300)))
#' @export
summary.pwr_sim <- function(sim, outcome, params=NULL, func=mean) {
    if (is.null(params)) {
        grid <- sim$tests
